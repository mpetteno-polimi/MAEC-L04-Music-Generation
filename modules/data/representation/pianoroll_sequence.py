...
class PianorollSequence(events_lib.EventSequence):
    """Stores a polyphonic sequence as a pianoroll. TODO - Modify DOC
    ...
    def __init__(self, quantized_sequence=None, events_list=None,
                 ...
                 max_pitch=MAX_MIDI_PITCH, split_repeats=False, shift_range=False):
    ...
    @staticmethod
    def _from_quantized_sequence(quantized_sequence, start_step, min_pitch, max_pitch, split_repeats):
        ...
        n_steps = quantized_sequence.total_quantized_steps - start_step
        n_pitches = max_pitch - min_pitch + 1

        piano_roll = np.zeros((n_steps, n_pitches, 2), np.int)
        ...
        for note in quantized_sequence.notes:
            ...
            # Populate play channel
            piano_roll[note_start_offset:note_end_offset, note_pitch_offset, 0] = note.velocity

            # Populate repeat channel
            is_repeated = 1 if piano_roll[note_start_offset - 1, note_pitch_offset, 0] > 0 else 0
            piano_roll[note_start_offset, note_pitch_offset, 1] = is_repeated

        events = []
        for frame in piano_roll:
            active_pitches = np.where(frame)[0]
            events.append(tuple(PianorollEvent(
                pitch=active_pitch,
                velocity=frame[active_pitch, 0],
                is_repeated=frame[active_pitch, 1]
    ...
    def to_sequence(self,
        ...
        for step, event in enumerate(self):
            frame_pitches = set([pianoroll_event.pitch for pianoroll_event in event])
            repeated_pitches = set([x.pitch for x in event if x.is_repeated == 1])
            open_pitches = set(open_notes)

            for pitch_to_close in (open_pitches - frame_pitches).union(repeated_pitches):
            ...
            for pitch_to_open in (frame_pitches - open_pitches).union(repeated_pitches):
                ...
                new_note.velocity = next(filter(lambda x: x.pitch == pitch_to_open, event)).velocity
...
class PianorollEvent(object):

    def __init__(self, pitch: int, velocity: int, is_repeated: bool):
        self.pitch = pitch
